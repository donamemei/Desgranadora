...
class EstadisticasController extends Controller
{
    public function index(): View
    {
        ...
        $mesExpresion = match ($driver) {
            'sqlite' => "strftime('%Y-%m', created_at) as mes",
            default => "DATE_FORMAT(created_at, '%Y-%m') as mes",
        };
        ...
        $stats = [
            ...
            'por_mes' => Lectura::selectRaw("{$mesExpresion}, MAX(kg_procesados) as kg, COUNT(DISTINCT sesion_id) as sesiones")
                ->groupBy('mes')
                ->orderBy('mes', 'desc')
                ->limit(6)
                ->get(),
